...
class TimeSeriesPredictor(ABC):
    ...
    def backtest(self, data: TimeSeries, start: float, forecast_horizon: int, 
                stride: int = 1, retrain: bool = False) -> Dict[str, Any]:
        ...
        try:
            ...
            historical_forecasts = self._generate_historical_forecasts(
                scaled_data, start, forecast_horizon, stride, retrain
            )
    ...
    @abstractmethod
    def _generate_historical_forecasts(self, series: TimeSeries, start: float, 
                                     forecast_horizon: int, stride: int, 
                                     retrain: bool) -> TimeSeries:
